...
const PlaceDetailsCompact = forwardRef<
  HTMLDivElement,
  PlaceDetailsCompactProps
>(({ data, startDate, days, isAdmin, skipped }, ref) => {
  ...
  const noteHeightRef = useRef<HTMLDivElement | null>(null);
            ...
            >
              ...
              {data.placeInfo.openingHours && (
                <div className="text-xs font-medium text-slate-700">
                  {data.placeInfo.openingHours.text[dayIndex].split(": ")[1]}
                </div>
              )}
            </button>
            {(!isAdmin || skipped) && (
